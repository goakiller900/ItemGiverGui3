#!/usr/bin/env python3
"""Build a Factorio-ready release archive from info.json metadata."""
...
def load_metadata() -> dict[str, object]:
    ...
    if not NAME_PATTERN.fullmatch(name):
        fail(
            "info.json name may contain only letters, digits, underscores and hyphens"
        )
    if not VERSION_PATTERN.fullmatch(version):
        fail("info.json version must use semantic form MAJOR.MINOR.PATCH")
...
def add_file_to_zip(
    archive: zipfile.ZipFile, source: Path, archive_path: PurePosixPath
) -> None:
...
def build_archive(metadata: dict[str, object], files: list[Path]) -> tuple[Path, Path]:
    name = str(metadata["name"])
    version = str(metadata["version"])
